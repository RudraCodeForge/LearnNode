...
const server = http.createServer((req, res) => {
  ...
  if (req.url === "/") {
    ...
    req.on("end", () => {
     ...
     // fs.writeFileSync("message.txt", JSON.stringify(dataObject)); //Write the data to a file using sync (This process is block the all other process or code.)


      fs.writeFile('message.txt',JSON.stringify(dataObject),error=>{
        console.log("Data Written sucessfully");
      }) //Write file without using sync because they did not block the other code or process
